...
After a runbook action, the orchestrator must NOT trust the script's exit code
alone.  Instead, it polls Prometheus for real metrics (p99 latency, up status)
and compares them against the thresholds defined in baseline.json.

The verify step requires min_samples consecutive healthy reads within the
timeout window to pass.  If the deadline expires, verify fails and triggers
an auto-rollback in the caller.
"""
...
def query_prometheus(prometheus_url: str, promql: str) -> float | None:
    """Execute an instant PromQL query and return the scalar value, or None."""
    try:
        resp = requests.get(
