#!/usr/bin/env bash
# clear_cache.sh — clear in-memory cache of a service by sending SIGHUP
#
# Usage:
#   bash clear_cache.sh --service <name> [--dry-run]
#
# In this lab the mock services don't have a real cache, so the script sends
# a SIGHUP to the container process (convention: many servers reload config /
# flush caches on SIGHUP) and logs the action.
#
# Exit codes:
