...
So sánh phân phối features giữa reference (baseline) và current (production window).
Flag drift khi dataset-level drift score vượt threshold.
...
Usage:
    ...
    # Combined (default): both data + performance checks:
    uv run python drift_detector.py \\
        --reference data/baseline.csv \\
        --current   data/drifted.csv \\
        --check-mode combined \\
...
@dataclass
class DriftResult:
    """Kết quả drift detection: data drift + optional performance drift."""
    ...
    report_path: str
    timestamp: str
    # Performance check fields (populated bởi check_performance_drift)
...
def detect_drift(
    ...
    # Lấy share of drifted columns và per-feature details
...
def main():
    parser = argparse.ArgumentParser(
        description="Detect data drift between two CSVs"
    )
    parser.add_argument(
    ...
    if args.check_mode in ("data", "combined"):
        ...
        # performance-only mode: stub result
        ts = datetime.utcnow().strftime("%Y%m%d-%H%M%S")
        result = DriftResult(
            score=0.0,
            is_drift=False,
    ...
    if args.check_mode in ("performance", "combined"):
        if not args.labeled_current:
            parser.error(
                "--labeled-current is required for performance/combined mode"
            )
    ...
    # Combined: drift nếu data drift HOẶC performance degradation
    ...
    # Push metrics lên Prometheus Pushgateway (no-op nếu gateway offline)
    try:
    ...
    # Exit code 1 nếu drift detected — hỗ trợ shell scripting
